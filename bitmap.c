...
//Cria uma copia do bitmap passado
bitmap* bitmapCopia(bitmap* bm) {
	bitmap* bmCopia = bitmapInit(bm->max_size);
	
	for(int i = 0; i < bm->length; i++){
		bitmapAppendLeastSignificantBit(bmCopia, bitmapGetBit(bm, i));
	}

	return bmCopia;
}

bitmap* bitmapLimpa(bitmap* bm) {
	int n = bm->max_size;
...
void bitmapSetByte(bitmap* bm, unsigned char ch){
	bm->contents[0] = ch;
	bm->length += 8;
}
